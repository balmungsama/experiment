...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Volumes/Pile Drive/OneDrive - University of Toronto/Labs/Inzlicht Lab/outside_project/experiment/final_version/final_rsvp.psyexp',
...
# stim_count  = 0 # keep track of which stimulus to display
    ...
    try:
        exp_tab = np.vstack((exp_tab, trial_tab))
    except NameError:
        exp_tab = trial_tab


df = pd.DataFrame(exp_tab)
df.to_csv("test.csv", header = ['block', 'sep_dur', 'Tx', 'trial', 'stimulus', 'cond'], index = False)

nreps_tot = np.shape(exp_tab)[0]

##########disp_stimulus #############
# stim_cur = stim_array[stim_count] #begin routine
# stim_count = stim_count + 1 #end routine

# Initialize components for Routine "disp_stimulus"
disp_stimulusClock = core.Clock()
stimulation = visual.TextStim(win=win, name='stimulation',
    text='default text',
    font=u'Arial',
...
for thisTrial in trials:
    ...
    # update component parameters for each repeat
    stimulation.setText(stimulus)
    # keep track of which components have finished
    disp_stimulusComponents = [stimulation, mask]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *stimulation* updates
        if t >= 0.0 and stimulation.status == NOT_STARTED:
            # keep track of start time/frame for later
            stimulation.tStart = t
        ...
        if stimulation.status == STARTED and t >= frameRemains:
